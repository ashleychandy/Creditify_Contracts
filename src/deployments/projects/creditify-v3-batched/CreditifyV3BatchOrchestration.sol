...
library CreditifyV3BatchOrchestration {
    ...
    function deployCreditifyV3(
        ...
        // StataToken deployment removed - not needed for this implementation
    ...
    function _deployGettersBatch2(
        ...
        CreditifyV3GettersBatchTwo gettersBatch2 =
            new CreditifyV3GettersBatchTwo(poolProxy, poolAdmin, wrappedNativeToken, poolAddressesProvider);
        CreditifyV3GettersBatchTwo.GettersReportBatchTwo memory gettersReportTwo = gettersBatch2.getGettersReportTwo();

        if (wrappedNativeToken != address(0)) {
            return gettersReportTwo;
        }

        return CreditifyV3GettersProcedureTwo.GettersReportBatchTwo({
            wrappedTokenGateway: address(0),
            protocolDataProvider: gettersReportTwo.protocolDataProvider,
            liquidationDataProvider: gettersReportTwo.liquidationDataProvider
        });
    }
    ...
    // REMOVED: _deployHelpersBatch2() - StataToken deployment not needed
    ...
    function _deployPeripherals(
        ...
        CreditifyV3PeripheryBatch peripheryBatch =
            new CreditifyV3PeripheryBatch(roles.poolAdmin, config, poolAddressesProvider, setupBatch);
    ...
    function _deployTokens(address poolProxy, address rewardsControllerProxy, PeripheryReport memory peripheryReport)
        internal
        returns (CreditifyV3TokensBatch.TokensReport memory)
    {
        address treasury = peripheryReport.treasury;
        if (peripheryReport.revenueSplitter != address(0)) {
            treasury = peripheryReport.revenueSplitter;
        }
        CreditifyV3TokensBatch tokensBatch = new CreditifyV3TokensBatch(poolProxy, rewardsControllerProxy, treasury);
    ...
    function _generateMarketReport(
        ...
        report.liquidationDataProvider = gettersReportTwo.liquidationDataProvider;

...
import {CreditifyV3DefaultRateStrategyProcedure} from
    "../../../contracts/procedures/CreditifyV3DefaultRateStrategyProcedure.sol";
    ...
    CreditifyV3IncentiveProcedure
{
    ...
    constructor(address poolAdmin, MarketConfig memory config, address poolAddressesProvider, address setupBatch) {
        ...
        if (
            ...
            _report.revenueSplitter =
                address(new RevenueSplitter(_report.treasury, config.treasuryPartner, config.treasurySplitPercent));
        }

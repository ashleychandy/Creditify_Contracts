...
contract CreditifyV3SetupProcedure {
    ...
    function _setupCreditifyV3Market(
        Roles memory roles,
        ...
        report.aclManager = _setupACL(roles, initialReport.poolAddressesProvider, report.poolConfiguratorProxy);
    ...
    function _setupPoolAddressesProvider(AddressProviderInput memory input) internal returns (SetupReport memory) {
        ...
        provider.setPriceOracle(input.creditifyOracle);
    ...
    function _setupACL(Roles memory roles, address poolAddressesProvider, address poolConfiguratorProxy)
        internal
        returns (address)
    {

...
The StataToken in an [EIP-4626](https://eips.ethereum.org/EIPS/eip-4626) generic token vault/wrapper intended to be used with creditify v3 aTokens.

## Features

- **Full [EIP-4626](https://eips.ethereum.org/EIPS/eip-4626) compatibility.**
- **Accounting for any potential liquidity mining rewards.** Let’s say some team of the Creditify ecosystem (or the Creditify community itself) decides to incentivize deposits of USDC on Creditify v3 Ethereum. By holding `stataUSDC`, the user will still be eligible for those incentives.
  It is important to highlight that while currently the wrapper supports infinite reward tokens by design (e.g. CREDITIFY incentivizing stETH & Lido incentivizing stETH as well), each reward needs to be permissionlessly registered which bears some [⁽¹⁾](#limitations).
- **Upgradable by the Creditify governance.** Similar to other contracts of the Creditify ecosystem, the Level 1 executor (short executor) will be able to add new features to the deployed instances of the `StataTokens`.
- **Powered by a StataTokenFactory.** Whenever a token will be listed on Creditify v3, anybody will be able to call the StataTokenFactory to deploy an instance for the new asset, permissionless, but still assuring the code used and permissions are properly configured without any extra headache.

See [IStata4626LM.sol](./interfaces/IERC20CreditifyLM.sol) for detailed method documentation.

## Deployed Addresses

The StataTokenFactory is deployed for all major Creditify v3 pools.
An up to date address can be fetched from the respective [address-book pool library](https://search.oncreditify.com/?q=stata%20factory).

## Limitations

The `stataToken` is not natively integrated into the creditify protocol and therefore cannot hook into the emissionManager.
...
1. `ERC20CreditifyLM` is an abstract contract implementing the forwarding of liquidity mining from an underlying CreditifyERC20 - an ERC20 implementing `scaled` functions - to holders of a wrapper contract.
...
2. `ERC4626StataToken` is an abstract contract implementing the [EIP-4626](https://eips.ethereum.org/EIPS/eip-4626) methods for an underlying aToken.
   ...
   In addition it adds a `latestAnswer` priceFeed, which returns the share price based on how creditify prices the underlying.
3. `StataTokenV2` is the main contract inheriting `ERC20CreditifyLM` and `ERC4626StataToken`, while also adding `Pausability`, `Rescuability`, `Permit` and the actual initialization.
...
- `underlying_price` is fetched from the CreditifyOracle, which means it is subject to mechanisms implemented by the DAO on top of the Chainlink price feeds.
...
- [Certora security review and property checking](https://github.com/creditify-dao/creditify-v3-origin/blob/067d29eb75115179501edc4316d125d9773f7928/audits/11-09-2024_Certora_StataTokenV2.pdf)

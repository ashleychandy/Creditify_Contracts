...
import {
    ICreditifyV3ConfigEngine as IEngine,
    IPoolConfigurator,
    IPool,
    IDefaultInterestRateStrategyV2
} from "../ICreditifyV3ConfigEngine.sol";
...
library ListingEngine {
    ...
    function executeCustomAssetListing(
        ...
        engineLibraries.borrowEngine.functionDelegateCall(
            abi.encodeWithSelector(BorrowEngine.executeBorrowSide.selector, engineConstants, repacked.borrowsUpdates)
        );
        ...
        returns (IEngine.RepackedListings memory)
    {
        ...
        for (uint256 i = 0; i < listings.length; i++) {
            ...
            basics[i] =
                IEngine.Basic({assetSymbol: listings[i].base.assetSymbol, implementations: listings[i].implementations});
            priceFeedsUpdates[i] =
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            initReserveInputs[i] = ConfiguratorInputTypes.InitReserveInput({
                ...
                variableDebtTokenName: string.concat(
                    "Creditify ", context.networkName, " Variable Debt ", basics[i].assetSymbol
                ),
                variableDebtTokenSymbol: string.concat("variableDebt", context.networkAbbreviation, basics[i].assetSymbol),
                params: bytes("")
            });
        }

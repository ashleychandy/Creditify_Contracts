...
library SupplyLogic {
    ...
    function executeSupply(
        ...
        if (isFirstSupply) {
            if (ValidationLogic.validateAutomaticUseAsCollateral(userConfig, reserveCache.reserveConfiguration)) {
        ...
        if (userConfig.isUsingAsCollateral(reserve.id)) {
            ...
            if (userConfig.isBorrowingAny()) {
                ValidationLogic.validateHFAndLtvzero(
                    reservesData, reservesList, userConfig, params.asset, params.user, params.oracle
                );
            }
        }
        ...
        if (params.from != params.to && params.scaledAmount != 0) {
            ...
            if (fromConfig.isUsingAsCollateral(reserveId)) {
                ...
                if (fromConfig.isBorrowingAny()) {
                    ValidationLogic.validateHFAndLtvzero(
                        reservesData, reservesList, usersConfig[params.from], params.asset, params.from, params.oracle
                    );
                }
            }
    ...
    function executeUseReserveAsCollateral(
        ...
        if (useAsCollateral) {
            ...
            require(
                ValidationLogic.validateUseAsCollateral(userConfig, reserveConfigCached), Errors.LtvValidationFailed()
            );

            userConfig.setUsingAsCollateral(reserve.id, asset, user, true);
        } else {
            userConfig.setUsingAsCollateral(reserve.id, asset, user, false);
            ValidationLogic.validateHFAndLtvzero(reservesData, reservesList, userConfig, asset, user, priceOracle);
        }
    }
}

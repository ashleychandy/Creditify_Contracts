...
library LiquidationLogic {
    ...
    function executeLiquidationCall(
        ...
        vars.borrowerCollateralBalance = IAToken(vars.collateralReserveCache.aTokenAddress).scaledBalanceOf(
            params.borrower
        ...
        if (vars.liquidationProtocolFeeAmount != 0) {
            uint256 scaledDownLiquidationProtocolFee = vars.liquidationProtocolFeeAmount.getATokenTransferScaledAmount(
        ...
        if (liquidatorPreviousATokenBalance == 0) {
            ...
            if (
                ValidationLogic.validateAutomaticUseAsCollateral(
                    liquidatorConfig, vars.collateralReserveCache.reserveConfiguration
                )
            ) {
